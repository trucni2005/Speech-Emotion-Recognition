...
from feature_extraction.scripts.extract_original_features import FeatureExtractor
...
def statistic_feature_extracting(n_mfcc, input_csv, output_csv):
    ...
    """
    feature_extractor = FeatureExtractor(n_mfcc)
...
if __name__ == '__main__':
    config = Config()
    n_mfcc_original = config.n_mfcc_original
    input_csv = config.cleaned_data_with_pad_and_trim_path_and_label_csv
    output_csv = config.feature_csv

    datasets = ['train', 'test', 'validation']
    for dataset_type in datasets:
        run_feature_extraction_for_dataset(dataset_type, n_mfcc_original, input_csv, output_csv)
